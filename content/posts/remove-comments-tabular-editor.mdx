...
```csharp
// Removes comments from the DAX expressions of SELECTED measures and
// SELECTED calculated columns only. Select the objects you want to clean
// in the model tree (Ctrl/Shift+click for multiple) before running this.
//
...
// NOTE: declared as a Func, not a method. Tabular Editor 2 compiles the script
// as a method body, so a real method declaration would be a local function
...
foreach (var col in Selected.Columns.OfType<CalculatedColumn>())
{
...
Info("Removed comments from " + changed + " selected object(s).");
```
