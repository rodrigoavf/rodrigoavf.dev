...
string RemoveComments(string input, string keepPrefix)
{
    ...
    while (i < n)
    {
        ...
        if (c == '/' && i + 1 < n && input[i + 1] == '/')
        {
            ...
            continue; // skip (or already appended) the comment text
        }
...
// ---- Selected calculated columns (regular calculated columns, not calculated table columns) ----
...
Console.WriteLine("Removed comments from " + changed + " selected object(s).");
```
